...
export const DELTA_SLICE_KEYS = [
  ...
  'specReading',
  // §5.3 #10-4 오케스트라 — 프로젝트당 설정 한 벌 + 최근 런(요청 원문·고른 방안의 이유)이라 작지 않다.
  //   `getOrchestraSummary` 가 설정·런 목록의 참조로 메모해 **안 바뀐 프로젝트는 같은 객체를** 돌려주므로
  // §5.3 #10-5 설정 덜어내기 — 프로젝트당 설정 한 벌 + 최근 기록(덜어낸/배제한 칸 목록)이라 작지 않다.
  //   `getConfigTrimSummary` 가 설정·기록 목록의 참조로 메모해 **안 바뀐 프로젝트는 같은 객체를** 돌려주므로
  //   정지 상태에서는 통째로 빠진다. 설정·기록은 바뀔 때마다 새 객체로 갈아 끼워진다(제자리 수정 ❌).
  'configTrim',
  //   정지 상태에서는 통째로 빠진다. 설정·런은 바뀔 때마다 새 객체로 갈아 끼워진다(제자리 수정 ❌).
  'orchestra',
] as const;
