...
import { agentRuleShell, buildAgentCardCommonRules } from '@vibisual/shared';
import { runCodexTurn, stopCodexTurn } from './codexRunner.js';

it.runIf(process.env.VIBISUAL_CODEX_RULE_LIVE_TEST === '1')('real Codex executes its generated card protocol with TOKEN filtered out', async () => {
  const cwd = mkdtempSync(join(tmpdir(), 'vibisual-codex-rules-'));
  const subAgentId = `rule-live-${randomUUID()}`;
  const token = randomUUID();
  const received: unknown[] = [];
  const server = createServer(async (req, res) => {
    if (req.method !== 'POST' || req.url !== '/api/agent-review' || req.headers['x-vibisual-hook-token'] !== token) {
  ...
  const rules = buildAgentCardCommonRules({ serverBase, serverToken: token, agentId: 'live-agent', subAgentId, shell: agentRuleShell('codex', process.platform) });
  ...
  try {
    ...
    expect(received).toEqual([{ agentId: 'live-agent', subAgentId, changes: ['한글 왕복 확인'], checkpoints: ['테스트 서버 수신'] }]);
  } finally {
    clearTimeout(timer);
