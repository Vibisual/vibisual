  ...
  getSnapshot(now: number = Date.now()): ProjectInsuranceLedger[] {
    ...
    for (const [projectName, state] of this.ledgers) {
      out.push(this.build(
        ...
        // §5.26 (I) ⑤ — 보험 창의 실패 칩이 좁힌 눈금의 것만 세려면 이 집계가 **전선에도** 실려야
        //   한다(창은 전문을 받기 전까지 스냅샷 원장으로 그린다). 잘린 목록으로 클라가 다시 세면
        //   서버가 아는 수와 어긋난다(§3.1 · §7.23).
        true,
      ));
    }
    return out;
 ...
 *
 * **실패가 0 인 세션은 싣지 않는다.** 화면이 쓰는 축이 실패 칩(보험 창 「압축 기록」 갈피) 하나뿐이고, 압축은 겪지만
