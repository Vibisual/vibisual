...
import { CODEX_AUTH_LOGIN_POLL_INTERVAL_MS, CODEX_AUTH_LOGIN_TERM_ID, DEFAULT_AGENT_CONFIG } from '@vibisual/shared';
...
interface LoginAttempt {
  ...
  ending: boolean;
  dispose: () => void;
  pending?: Promise<CodexAuthStatus | null>;
}
...
export function useCodexLoginSession(
  ...
  const start = useCallback(async (binPath: string, mode: CodexAuthLoginMode): Promise<void> => {
    ...
    const termId = `${CODEX_AUTH_LOGIN_TERM_ID}:${nonce}`;
    const launch = buildCodexLoginCommand(binPath, mode, codexLoginPlatform(binPath));
    ...
    const offData = transport.onData((event) => {
      ...
      const next = scanLoginOutput(buffer, { deviceAuth: mode === 'device' });
      setScan(next);
    ...
    const timer = setInterval(() => { void check(attempt); }, CODEX_AUTH_LOGIN_POLL_INTERVAL_MS);
  ...
  }, [transport, release, finish, check]);
