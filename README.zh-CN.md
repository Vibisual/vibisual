...
也没有用量上限。源代码遵循 Apache-2.0；我们发布的安装包适用 [EULA](EULA.md)，
同样对个人和商业用途免费 —— 见[许可证](#许可证)。

之后可能会有付费的增值部分：额外的算力额度、跑在你自己机器之外的工作、为团队做的功能。

## 许可证

**源代码遵循 Apache License 2.0** —— 见 [LICENSE](LICENSE)。你可以阅读、修改、
自行构建、再分发。这项权利是永久的，不受下面条款影响。

**我们发布的安装包适用 [EULA](EULA.md)** —— 即 Releases 页面上的安装程序和
二进制文件。它对个人和商业用途都免费，机器数量不限，不需要账号、许可证密钥，
也没有用量上限。EULA 要求的是：不要再分发、重新打包或改换品牌来发布**我们的**
构建产物。如果你想发布自己的产品，请从源代码构建并使用你自己的名称。

"Vibisual" 与 Vibisual 标志是 길근오（项目所有者）的商标；政策见
